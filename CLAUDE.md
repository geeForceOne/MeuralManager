...
- **`src/MeuralManager.Web`** (`net10.0`, Blazor Server) — the UI, packaged for Docker via `Dockerfile`/`docker-compose.yml`:
  ...
  - `Components/Pages/` — `Dashboard` (scan trigger + status, tool shortcuts, "Picture of the Moment"), `Playlists` (resizable three-pane browser: sortable playlist table, item thumbnail grid, image preview), `OrphanUploads`, `UnusedGalleries`, `Settings`, `Immich` (three tabs — Photos timeline grouped by day with date/text filters, Albums, People — all showing the same paged photo grid; selection persists across tabs; uploads reuse the shared `CropDialog` and `PlaylistService.UploadAndAddToGalleryAsync`, and a photo that fails to upload stays selected for a retry).
  - `Components/Shared/` — small reusable pieces used across pages: `ConfirmDialog`/`PromptDialog` (imperative modal dialogs awaited from code; `ConfirmDialog.AskChoiceAsync` adds a second non-cancel option for questions like "skip those / upload again"), `AddExistingItemsModal` (searchable thumbnail picker), `ActivityLogPanel`, `Toast` (transient success/error banner), `CropDialog` (16:9/9:16 frame-overlay crop), `ImmichPhotoGrid` (day-grouped, shift-click range-select photo grid that loads more via `wwwroot/js/infinite-scroll.js`'s IntersectionObserver). Don't name a component parameter `Assets` — it collides with `ComponentBase.Assets` in .NET 10.
